...
def arg_parser():
    ...
    parser.add_argument(
        "--colID", type=str, default="", help="ID to prepend collections"
    )
    parser.add_argument(
        "--collect",
        type=int,
...
def print_reads(samp, reads_list, col_reads, args):
    ...
    for line in reads_list:
        if args.AAreport == 1:
            try:
                reads_fh.write(
                    f"{line[0]}\t{line[2]} {col_reads[line[1]]['AA_sequence']} {line[3]}\n"
                )
            except:
...
def print_unique_seq(samp, sam_read_count, col_reads, coverage, args):
    ...
    for SNP_sequence in col_reads:
        for start_end in col_reads[SNP_sequence]:
            if "x" in start_end:
                if args.wgs == 1:
                    seq_species[start_end + "x" + SNP_sequence] = col_reads[
                        SNP_sequence
                    ][start_end]
                else:
                    try:
            ...
            if abund >= args.min_samp_abund:
                try:
                    ...
                    if args.AAreport == 1:
                        try:
                            seq_fh.write(
                                f"{col_reads[key]['AA_sequence']}\t{seq_species[key]}\t{abund:.3f}\n"
                            )
                        except:
                    ...
                    if args.AAcentered == 1 and args.AAreport == 1:
                        try:
                            seq_fh.write(
                                f"{col_reads[key]['AA_centered']}\t{seq_species[key]}\t{abund:.3f}\n"
                            )
                        except:
                            if not key == "Reference":
                                print(
                                    "Non Reference seq without AA centereed output : "
                                    + splitseqs[2]
                                )
                else:
                    if args.AAreport == 1:
    ...
    for combinations, samp_tag in ((ntcombinations, ""), (AAcombinations, "_AA")):
        try:
            ...
            if max_count >= args.min_count:
                ...
                sortedcombos = sorted(
                    combinations, key=combinations.__getitem__, reverse=True
                )
            ...
            if args.AAreport == 1:
                ...
                for Pos in sorted_Pos:
                    ...
                    if (
                        ...
                        sorted_calls = sorted(
                            Pos_calls, key=Pos_calls.__getitem__, reverse=True
                        )
...
def gb_sam_parse(args, ref, file):
    ...
    if sam_read_count == 0:
        ...
        if args.AAreport == 1 and (
            args.seq == 1 or args.covar == 1 or args.indel == 1 or args.read == 1
        ):
            for SNP_sequence in col_reads:
                if "Reference" == SNP_sequence:
                    pass
                                    ...
                                    if len(entry) > 1:
                                        ...
                                        if nt_to_AA_key in nt_to_AA_dict[orf]:
                                            for SNP in nt_to_AA_dict[orf][nt_to_AA_key]:
                                                mutorfs[orf][SNP] = nt_to_AA_dict[orf][
                                                    nt_to_AA_key
                                                ][SNP]
                                        else:
                                            MNPreport = ""
                                            ...
                                            if endmod == 2:
                                                ...
                                                try:
                                                    ...
                                                    rf_end_del = 1
                                            wtntseq = ref[3][orf]["nts"][
                                                wtorfstartpos - 1 : wtorfendpos
                                            ]
                                            wtAAseq = ref[3][orf]["AAs"][
                                                startcodonpos - 1 : endcodonpos
                                            ]
                                            mutAAseq = ""
                                            for i in range(0, (len(mutntseq) // 3)):
                                            ...
                                            for curPM in entry:
                                                ...
                                                try:
                                                    nt_to_AA_dict[orf][nt_to_AA_key][
                                                        curPM[0]
                                                    ] = newmut
                                                except:
                                                    nt_to_AA_dict[orf][nt_to_AA_key] = {
                    ...
                    for mut in col_reads[SNP_sequence]["AA_sequence"].split(" "):
                        if "|" in mut:
                            ...
                            for orfmut in split_mut[1:]:
                                ...
                                startPos = int(
                                    split_mut[0].split("-")[0].strip("ATCGN")
                                )
                                endPos = startPos
                                if "del" in split_mut[0]:
                                    endPos = int(
                                        split_mut[0].split("-")[1].strip("del")
                                    )
        ...
        if args.covar == 1:
            print_covars(
                samp, sam_read_count, col_reads, coverage, args, aa_genome_pos_dict
            )
                    ...
                    if (
                        ...
                        try:
                            ...
                            for insertion in ins_nt_dict[Pos]:
                                ...
                                try:
                                    i_AAs = "|".join(
                                        gb_snp_call(insertion, ref).split("|")[1:]
                                    )
                                except:
                                    i_AAs = ""
                            ...
                            try:
                                for entry in OrfPosDict[Pos]:
                                    try:
                                        ...
                                        ORFmismatch[entry[0]] = {
                                            entry[1]: sorted_calls[0]
                                        }
                                    else:
                                        ORFmismatch[entry[0]][entry[1]] = sorted_calls[
                                            0
                                        ]
                            except:
                                pass
                    ...
                    try:
                        ...
                        if consensus[Pos][0] != ref[1][Pos - 1]:
                            ...
                            try:
                                ...
                                if (
                                    nt_call_dict_dict[Pos][consensus[Pos][1]]
                                    > args.min_count
                                ) and (
                                    nt_call_dict_dict[Pos][consensus[Pos][1]] / total
                                    > args.min_samp_abund
                                    ...
                                    if (
                                        nt_call_dict_dict[Pos][consensus[Pos][2]]
                                        > args.min_count
                                        and nt_call_dict_dict[Pos][consensus[Pos][2]]
                                        / total
                                        > args.min_samp_abund
                                    ):
                                        ntcall_lines["line"][Pos] += (
                                ...
                                for ORFentry in OrfPosDict[Pos]:
                                    ...
                                    orfAAreports[0].append(
                                        orf + "_" + aa_call("".join(codon))
                                    )
