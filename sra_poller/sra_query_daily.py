...
def parse_process_arrays_args(parser: ArgumentParser):
    ...
    parser.add_argument('--completed_files',
                        ...
                        required=True)
    parser.add_argument('--serpent_dir',
                        type=str,
                        help='serpent_dir',
                        required=True)
    parser.add_argument('--config_dir',
                        ...
                        help='config_dir',
                        required=True)
    parser.add_argument('--default_config_dir',
                        type=str,
                        help='default_config_dir',
                        required=True)
...
if True:
    ...
    print(f"curl -A 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:50.0) Gecko/20100101 Firefox/50.0' 'https://www.ncbi.nlm.nih.gov/sra/?term=sars-cov-2+wastewater' -o {search_path}")
    os.system(
...
df_all_remain_filtered = df_all_remain[df_all_remain[3] != 'national institute of biology']
all_filtered_list = set(df_all_remain_filtered[0].unique())
# Output the list to be used in the typical loop
df_all_remain_filtered.to_csv(os.path.join(local_module_in_dir, 'SRA_meta_run.tsv'), sep='\t', index=False, header=False)
# output the list for the skipped with meta data. This may be redundant but that is okay
df_skipped.to_csv(os.path.join(local_module_in_dir, 'SRA_meta_skipped.tsv'), sep='\t', index=False, header=False)
